...
>
  ...
  <!-- Table Body Row Styling - QUARTZ OVERRIDE -->
        ...
        <xsl:when test="$isColoredRow">
          <xsl:choose>
            <xsl:when test="$tableTheme">
              <!-- QUARTZ CHANGE: Use the main theme color, not -subtle -->
              <xsl:call-template name="processBootstrapAttrSetReflection">
                <xsl:with-param name="attrSet" select="concat('__table__', $tableTheme)"/>
              </xsl:call-template>
            </xsl:when>
  ...
  <!-- Entry styling - QUARTZ OVERRIDE for striped columns -->
  <xsl:template match="*[contains(@class, ' topic/entry ')]" priority="10">
    ...
    <fo:table-cell>
      ...
      <!-- Cell-level themed color or striping -->
      <xsl:choose>
        <xsl:when test="@color">
           <xsl:call-template name="processBootstrapAttrSetReflection">
