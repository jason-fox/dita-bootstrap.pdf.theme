...
>
  ...
  <!-- Table Header Row Styling - SPACELAB OVERRIDE -->
  <xsl:template match="*[contains(@class, ' topic/thead ')]/*[contains(@class, ' topic/row ')]" priority="10">
    <fo:table-row xsl:use-attribute-sets="thead.row">
      ...
      <xsl:if test="$theme">
        <!-- SPACELAB heading follow button color (reflected via __table__) -->
  ...
  <!-- Table Body Row Styling - SPACELAB OVERRIDE -->
  <xsl:template match="*[contains(@class, ' topic/tbody ')]/*[contains(@class, ' topic/row ')]" priority="10">
    <fo:table-row xsl:use-attribute-sets="tbody.row">
      ...
      <xsl:choose>
        <!-- Row-specific color wins - SPACELAB: USE SUBTLE -->
  ...
  <xsl:template match="*[contains(@class, ' topic/entry ')]" priority="10">
    ...
    <fo:table-cell>
      ...
      <!-- Cell-level themed color or striping - SPACELAB: USE SUBTLE -->
      <xsl:choose>
        <xsl:when test="@color">
           <xsl:call-template name="processBootstrapAttrSetReflection">
